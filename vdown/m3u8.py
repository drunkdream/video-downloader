...
class M3U8Downloader(object):
    ...
    def start_task(self, coroutine, delay=0):
        logger.debug(
            "[%s] Start task %s" % (self.__class__.__name__, coroutine.__name__)
        )
        ...
        async def _wrap():
            ...
            try:
                ...
                logger.info(
                    "[%s] Task %s exit" % (self.__class__.__name__, coroutine.__name__)
                )
            except:
                import traceback
                ...
                traceback.print_exc()
                logger.error(
                    "[%s] Task %s exit unexpectly"
                    % (self.__class__.__name__, coroutine.__name__)
                )
                self._running = False
        ...
        if self._video_count:
            ...
            while True:
                if (
                    self._recording
                    and record_time
                    and time.time() - time0 >= record_time
                ):
                ...
                print(
                    "\rDownloding %d/%d %s"
                    % (
                        len(self._ts_list) - self._running_tasks,
                        self._video_count,
                        "." * (index % 3 + 1) + "   ",
                    ),
                    end="",
